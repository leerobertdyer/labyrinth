...
export default function DialogueView({
  ...
  function handleResponse() {
    ...
    sendPrompt(nextHistory, conversation.systemPrompt)
      .then((resp) => {
        const dialogue = resp.reply.text;
        // const toolsCalled = resp.reply.calledTools; // TODO: open shop with STATE MACHINE CALL
        setNpcDialogue(dialogue);
      })
      .finally(() => {
        inFlight.current = false;
  ...
  return (
    ...
    >
      ...
      >
        {!showNext && <p className="text-gray-400 italic">{conversation.narration}</p>}
        {showNext ? (
          <>
          ...
          <button onClick={() => setShowNext(true)} className="w-25 rounded-sm border-2 border-white p-3 cursor-pointer hover:bg-white hover:text-black text-amber-400">
            Continue
          </button>
        )}
      </div>
